...
ALPHA_STEPS = 5
ALPHAS = np.linspace(0.02, 0.12, ALPHA_STEPS)
SIM_PN_RMS = np.zeros(ALPHA_STEPS)
EST_PN_RMS = np.zeros(ALPHA_STEPS)
for n, alpha in enumerate(ALPHAS):
    ...
    # Opt BBPD
    k = k_fixed_bw(alpha, FCLK)

    # Calculate LF for emergent BB behavior
    print("\nCalculating LF for Emergent BB behavior")
    int_pn = total_int_pn(s0, k, FOSC, BETA)
    print("\tComputed integrated PN = %f [rad^2]"%int_pn)
    ...
    # alpha = bw/fref
    print("\tBW = %E [Hz]"%bw)
    lf_bbpd = calc_lf_bbpd(KDCO1, k, int_pn)
    calc_discrete_lf(lf_bbpd, FCLK)
    lf_bbpd["bw"] = bw
    lf_bbpd["posc"] = DCO_POWER
    lf_bbpd["fosc"] = FOSC
    lf_bbpd["fom"] = DCO_FOM
    lf_bbpd["alpha"] = alpha
    lf_bbpd["rms_jit"] = 0
    ...
    lf_sc = calc_lf_sc(ALPHA_MAX, FCLK, FOSC, KDCO1)
    calc_discrete_lf(lf_sc, FCLK)
    lf_sc["bw"] = FCLK*ALPHA_MAX
    lf_sc["posc"] = DCO_POWER
    lf_sc["fosc"] = FOSC
    lf_sc["fom"] = DCO_FOM
    lf_sc["alpha"] = ALPHA_MAX
