...
lfs = design_filters(DCO_FOM, BBPD_RMS_JIT, FCLK, FOSC, ALPHA_MAX, DCO_POWER, KDCO1, alpha_max_sc=ALPHA_MAX_SC, beta=0.884)
...
plt.figure(1)
plt.subplot(3,3,1)
plot_td(sim_data["lf"], tmax=PLOT_SPAN, title="LF", dots=False)
plt.subplot(3,3,2)
plot_td(sim_data["scpd"], tmax=PLOT_SPAN, title="SCPD", dots=False)
plt.subplot(3,3,3)
plot_td(sim_data["bbpd"], tmax=PLOT_SPAN, title="BBPD", dots=False)
plt.subplot(3,3,4)
plot_td(meas_inst_freq(sim_data["osc"]), tmax=PLOT_SPAN, title="Freq", dots=False)
plt.subplot(3,3,5)
plot_td(sim_data["error"], tmax=PLOT_SPAN, title="Error", dots=False)
plt.subplot(3,3,6)
plot_td(sim_data["fine"], tmax=PLOT_SPAN, title="fine", dots=False)
plt.subplot(3,3,7)
plot_td(sim_data["med"], tmax=PLOT_SPAN, title="med", dots=False)
plt.subplot(3,3,8)
pn_sig = pn_signal2(sim_data, div_n=round(FOSC/FCLK))
pn_sig = debias_pn(pn_sig, 0.5*SIM_SPAN)
plot_td(pn_sig, tmax=PLOT_SPAN, title="Phase error", dots=False)
...
print("\nSimulated RMS jitter:\t\t%.3E ps"%(1e12*rms/(2*np.pi*FOSC)))
print("Filter design RMS jitter:\t%.3E ps"%(1e12*est_rms_pn_design/(2*np.pi*FOSC)))
...
plt.figure(2)
plot_pn_ssb2(pn_sig, dfmax=PN_FMAX, line_fit=False, tmin=GEAR_SW_T*PN_CALC_SAFETY_FACTOR, label="$\\phi_n$")
plot_pn_ar_model(pn_sig, p=200, fmin=FBIN, tmin=GEAR_SW_T*PN_CALC_SAFETY_FACTOR, label="$\\phi_n$")
# rpm = noise_power_ar_model(pn_sig, fmax=FCLK/2, p=200, tmin=GEAR_SW_T)
# print("int rpm = ", np.sqrt(rpm))


plot_pn_ar_model(em_pn, p=200, fmin=FBIN, tmin=GEAR_SW_T*PN_CALC_SAFETY_FACTOR, label="$\\phi_{em}$")
...
plt.yticks((0, 4096, 8192), ["0", "4096", "8192"])
plt.xticks(xticks, ["", "", "", "", ""])
ax = plt.gca()
...
plt.ylim((0,1024))
plt.ylabel("Fine")
plt.yticks((0, 512, 1024), ["0", "512", "1024"])
ax = plt.gca()
ax.axes.xaxis.label.set_visible(False)
plt.subplot(3,1,3)
plot_td(sim_data["med"], tmax=PLOT_SPAN, title="med", dots=False)
razavify(legend=False)
plt.xticks(xticks, ["%.0f"%(1e6*x) for x in xticks])
...
plt.figure(4)
plot_td(meas_inst_freq(sim_data["osc"]), tmax=PLOT_SPAN, title="Freq", dots=False)
plt.title("Oscillator Instantaneous Frequency")
plt.axhline(816e6, color="red", linestyle=":")
razavify()
plt.xticks(xticks, ["%.0f"%(1e6*x) for x in xticks])
plt.xlabel("Time [$\\mu$s]")
plt.ylabel("Frequency [MHz]")
plt.xlim((0,20e-6))
yticks = np.linspace(800e6, 824e6, 7)
plt.yticks(yticks, ["%.0f"%(1e-6*x) for x in yticks])
plt.ylim((800e6, 824e6))
plt.show()

print("BBPD b0 = %.10E"%lfs["bbpd"]["b0"])
print("BBPD b1 = %.10E"%lfs["bbpd"]["b1"])

# plt.scatter(em_pn.td[int(np.ceil(GEAR_SW_T*FCLK)):],pn_sig.td[int(np.ceil(GEAR_SW_T*FCLK)):])
# plt.show()
foo()
import pickle
with open("./bbpd_pipll_wave.pickle", "wb") as f:
